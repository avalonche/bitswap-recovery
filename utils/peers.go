...
func DialOtherPeers(ctx context.Context, self core.Host, ais []peer.AddrInfo, unavailableCount int) ([]peer.AddrInfo, int, error) {
	...
	for _, ai := range toDial {
		...
		g.Go(func() error {
			if err := self.Connect(ctx, ai); err != nil {
				failedConnections++
				if failedConnections > unavailableCount {
	...
	if err := g.Wait(); err != nil {
		return nil, failedConnections, err
	}

	return toDial, failedConnections, nil
}